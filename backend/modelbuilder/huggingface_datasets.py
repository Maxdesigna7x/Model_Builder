...
DATASETS: dict[str, dict] = {
    ...
    "ecg-arrhythmia": {
        ...
        "defaultOptions": {"normalization": "none", "batchSize": 32},
        "loader": "ecg",
    },
    "mnist": {
        "name": "MNIST",
    ...
    "wikitext-2": {
        ...
        "defaultOptions": {"max_length": 64, "vocab_size": 4096, "batchSize": 32, "max_samples": 20000},
        "loader": "wikitext",
    },
    "monuseg": {
        "name": "MoNuSeg",
...
def _load_energy(spec: dict, task_id: str, opts: dict, offline: bool):
    ...
    if task_id == "tabular.regression":
        ...
        starts = range(0, len(values) - window - horizon + 1, stride)
        x = torch.stack([values[start:start + window] for start in starts])
        future = torch.stack([target[start + window:start + window + horizon] for start in range(0, len(values) - window - horizon + 1, stride)])
        y = future.mean(1) if task_id == "sequence.regression" else future
    normalization = str(opts.get("normalization", "none"))
    if normalization != "none":
        x = _apply_normalization(x, normalization)
        if task_id == "tabular.reconstruction":
            y = x.clone()
    return x, y, None


def _load_ecg(spec: dict, task_id: str, opts: dict, offline: bool):
    path = _hf_file(spec, "sample_beats.csv", offline)
    with path.open(newline="", encoding="utf-8-sig") as handle:
        rows = list(csv.DictReader(handle))
    features = [str(index) for index in range(180)]
    classes = ["Normal", "Bloqueo de rama izquierda", "Bloqueo de rama derecha", "Contracción ventricular prematura", "Contracción auricular prematura"]
    raw_labels = ["N", "L", "R", "V", "A"]; mapping = {label: index for index, label in enumerate(raw_labels)}
    x = torch.tensor([[[float(row[name])] for name in features] for row in rows], dtype=torch.float32)
    y = torch.tensor([mapping[row["label"]] for row in rows], dtype=torch.long)
    return x, y, classes


def _load_mnist(spec: dict, task_id: str, opts: dict, offline: bool):
    dataset = _hf_dataset(spec, split="train", offline=offline)
    limit = min(len(dataset), int(opts.get("max_samples", 12000))); resolution = int(opts["resolution"]); channels = int(opts["channels"])
    ...
    return x, torch.tensor(rows["label"], dtype=torch.long), [str(index) for index in range(10)]
...
def _load_utkface(spec: dict, task_id: str, opts: dict, offline: bool):
    ...
    return torch.stack(images), torch.tensor(ages, dtype=torch.float32).unsqueeze(1), None


def _tokenize_texts(texts: list[str], opts: dict, labels: torch.Tensor | None = None):
    import re
    words = [re.findall(r"\w+|[^\w\s]", text.lower(), flags=re.UNICODE) for text in texts]
    counts: dict[str, int] = {}
    for row in words:
        for word in row: counts[word] = counts.get(word, 0) + 1
    max_vocab = int(opts.get("vocab_size", 4096)); vocab = ["<pad>", "<unk>"] + [word for word, _ in sorted(counts.items(), key=lambda item: (-item[1], item[0]))[:max_vocab - 2]]
    ...
    for index, row in enumerate(words):
        ids = [lookup.get(word, 1) for word in row[:length]]
...
def _load_emotion(spec: dict, task_id: str, opts: dict, offline: bool):
    ...
    texts = [str(text) for part in parts for text in part["text"]]
    labels = torch.tensor([int(label) for part in parts for label in part["label"]], dtype=torch.long)
    x, y, vocab = _tokenize_texts(texts, opts, labels)
    opts["vocab"] = vocab
    return x, y, ["Tristeza", "Alegría", "Amor", "Ira", "Miedo", "Sorpresa"]
...
def _load_wikitext(spec: dict, task_id: str, opts: dict, offline: bool):
    ...
    texts = [str(text).strip() for part in parts for text in part["text"] if str(text).strip()]
    texts = texts[:int(opts.get("max_samples", 20000))]
    x, _, vocab = _tokenize_texts(texts, opts)
...
def _load_segmentation(spec: dict, task_id: str, opts: dict, offline: bool):
    ...
    if task_id.endswith("binary"):
        return x, (masks > 0).long(), ["Objeto"]
    # Ambiguous class 5 is not a scored MoNuSAC class; fold it into background.
    masks[masks == 5] = 0
    return x, masks, ["Fondo", "Epitelial", "Linfocito", "Macrófago", "Neutrófilo"]
...
def download(project: dict, task_id: str, dataset_id: str, options: dict | None = None) -> dict:
    ...
    try:
        x, y, classes = loader(spec, task_id, opts, offline)
    except BackendError:
        if not offline:
            raise
        # A stale marker must not make the feature permanently unusable.
        marker.unlink(missing_ok=True)
        x, y, classes = loader(spec, task_id, opts, False)
    ...
    summary = _save(
        ...
        metadata={
            ...
            "optionsSignature": _signature(opts), "cacheStatus": "disk" if offline else "downloaded",
        },
    )
    return summary
