...
def split_indices(n: int, seed: int, percentages: dict[str, int] | None = None) -> dict[str, list[int]]:
    ...
    if train_percent + validation_percent + test_percent != 100 or min(train_percent, validation_percent, test_percent) < 1:
        raise ValueError("Las particiones deben ser positivas y sumar 100%")
...
def _save(project: dict, task_id: str, x: torch.Tensor, y: torch.Tensor, description: str, seed: int, classes: list[str] | None = None, options: dict | None = None) -> dict:
    ...
    summary = {
        "id": dataset_id, "revision": 1, "source": "synthetic", "samples": len(x),
...
def apply_pipeline(project: dict, dataset: dict, task_id: str, pipeline: dict) -> dict:
    ...
    for index,node in enumerate(nodes):
        ...
        if split_position>=0 and node_type in {"normalize","image_normalize","clip"} and index < split_position:
            raise ValueError(f"{node.get('label',node_type)} debe ejecutarse después de Particiones")
        if node.get("category")=="augment" and node.get("scope")!="train":
            raise ValueError("Las aumentaciones deben estar limitadas a train")
        if node_type=="random_flip" and task_id=="image.regression":
            raise ValueError("Flip aleatorio no es seguro para regresión visual sin transformar también el target")
        current_stage=stage_order.get(node.get("category"),last_stage)
        if current_stage<last_stage and node_type!="output": raise ValueError(f"{node.get('label',node_type)} está fuera de orden")
    ...
    for node in nodes:
        ...
        if kind=="validate":
            ...
            if count: diagnostics.append({"level":"error" if props.get("fail_on_error",True) else "warning","nodeId":node_id,"message":f"{count} muestras contienen valores no finitos."})
            else: diagnostics.append({"level":"info","nodeId":node_id,"message":f"{len(x)} muestras verificadas; shapes y valores son válidos."})
            has_later_filter=any(later.get("type")=="finite_filter" for later in nodes[nodes.index(node)+1:])
            if count and props.get("fail_on_error",True) and not has_later_filter: raise ValueError(f"La validación encontró {count} muestras con NaN o infinito; añade Excluir no finitos")
            ...
            removed=int((~keep).sum());x,y=x[keep],y[keep];splits=None;diagnostics.append({"level":"info","nodeId":node_id,"message":f"Se excluyeron {removed} muestras no finitas."})
            ...
            removed=len(x)-len(keep);x,y=x[keep],y[keep];splits=None;diagnostics.append({"level":"info","nodeId":node_id,"message":f"Se eliminaron {removed} duplicados exactos."})
        elif kind=="split":
            ...
            if sum(percentages.values())!=100 or min(percentages.values())<1: raise ValueError("Las particiones deben ser positivas y sumar 100%")
            if strategy=="temporal" and not task_id.startswith("sequence"): raise ValueError("La partición temporal sólo admite secuencias")
            ...
            if not 0<=lower<upper<=1: raise ValueError("Los cuantiles deben cumplir 0 ≤ inferior < superior ≤ 100")
            ...
            if not task_id.startswith("image"): raise ValueError("Redimensionar sólo admite imágenes")
            ...
            if not task_id.startswith("image"): raise ValueError("Recorte central sólo admite imágenes")
            height=max(8,int(props.get("height",96)));width=max(8,int(props.get("width",96)));source_h,source_w=x.shape[-2:]
            if height>source_h or width>source_w: raise ValueError(f"El recorte {width}×{height} supera la imagen {source_w}×{source_h}")
            ...
            if not task_id.startswith("image"): raise ValueError("Convertir canales sólo admite imágenes")
    ...
    if len(x)<3: raise ValueError("El pipeline dejó menos de tres muestras")
    ensure_splits()
...
def import_dataset(project: dict, task_id: str, path: str, options: dict | None = None) -> dict:
    ...
    if not source.exists(): raise ValueError("La ruta seleccionada no existe")
    opts = options or {}
    spec = task_spec(task_id)
    if task_id.startswith("tabular"):
        if source.suffix.lower() != ".csv": raise ValueError("Esta tarea requiere un archivo CSV")
        with source.open(newline="", encoding="utf-8-sig") as handle: rows = list(csv.DictReader(handle))
        if len(rows) < 10: raise ValueError("El CSV necesita al menos 10 filas")
        ...
        except (ValueError, KeyError) as exc: raise ValueError("Las variables deben ser numéricas en esta versión") from exc
        norm = opts.get("normalization", "none")
    ...
    if task_id.startswith("text."):
        if source.suffix.lower() not in {".txt", ".csv", ".tsv"}: raise ValueError("Texto requiere .txt, .csv o .tsv")
        ...
        if task_id == "text.classification":
            ...
            if len(pairs)<10: raise ValueError("Clasificación de texto requiere al menos 10 líneas label<TAB>texto")
            ...
            if len(texts)<10: raise ValueError("El corpus necesita al menos 10 líneas no vacías")
            label_names=[]; y=None
    ...
    if task_id.startswith("sequence"):
        if source.suffix.lower() != ".csv": raise ValueError("Esta tarea requiere un CSV largo")
        with source.open(newline="", encoding="utf-8-sig") as handle: rows = list(csv.DictReader(handle))
        required = {"sequence_id", "timestep"}
        if not rows or not required.issubset(rows[0]): raise ValueError("Faltan las columnas sequence_id y timestep")
        ...
        if len(lengths) != 1 or min(lengths) < 2: raise ValueError("Las secuencias deben tener la misma longitud y al menos dos pasos")
        ...
        if not features: raise ValueError("No se encontraron variables de entrada")
        try: x = torch.tensor([[[float(row[col]) for col in features] for row in values] for values in groups.values()], dtype=torch.float32)
        except ValueError as exc: raise ValueError("Las variables de secuencia deben ser numéricas") from exc
        norm = opts.get("normalization", "none")
        if norm != "none": x = _apply_normalization(x, norm)
        last_rows = [values[-1] for values in groups.values()]
        if spec["objective"] == "classification":
            if "target" not in rows[0]: raise ValueError("Clasificación requiere la columna target")
            ...
            if not future_cols: raise ValueError("Pronóstico requiere columnas future_1, future_2, … en cada secuencia")
            ...
            if "target" not in rows[0]: raise ValueError("Regresión requiere la columna target")
    ...
    if task_id in {"image.classification", "image.regression", "image.reconstruction"}:
        ...
        if task_id.endswith("classification"):
            ...
            if len(class_dirs) < 2: raise ValueError("Se requieren al menos dos carpetas de clase con imágenes")
            image_paths=[]; labels=[]
            ...
            if not labels_path.is_file(): raise ValueError("Regresión visual requiere labels.csv con filename,target")
            with labels_path.open(newline="",encoding="utf-8-sig") as handle: rows=list(csv.DictReader(handle))
            if not rows or not {"filename","target"}.issubset(rows[0]): raise ValueError("labels.csv requiere filename y target")
            paths=[(root/row["filename"]).resolve() for row in rows]
            if not all(path.is_file() and _is_image(path) for path in paths): raise ValueError("labels.csv referencia una imagen ausente o incompatible")
            ...
            if len(image_paths)<10: raise ValueError("Reconstrucción visual requiere al menos 10 imágenes")
    ...
    if "segmentation" in task_id:
        ...
        resolution = int(opts.get("resolution", 64))
        channels = int(opts.get("channels", 3))
        if not images_dir.is_dir() or not masks_dir.is_dir(): raise ValueError("Segmentación requiere carpetas images/ y masks/")
        pairs=[]
        for image_path in sorted(file for file in images_dir.iterdir() if _is_image(file)):
            ...
            if len(candidates)!=1: raise ValueError(f"La imagen {image_path.name} necesita exactamente una máscara")
            pairs.append((image_path,candidates[0]))
        if len(pairs)<10: raise ValueError("Se requieren al menos 10 pares imagen/máscara")
        ...
        if task_id.endswith("binary"):
            ...
            if len(values)<2: raise ValueError("Las máscaras multiclase contienen menos de dos clases")
    ...
    raise ValueError("No existe un importador para esta tarea")
...
def _load_image(path: Path, resolution: int = 256, channels: int = 3) -> torch.Tensor:
    try:
        ...
        raise ValueError(f"No se pudo leer la imagen {path.name}") from exc
    if channels == 1:
