...
export function templateFor(architecture: Architecture, task: TaskId): { nodes: ModelNode[]; edges: ModelEdge[] } {
  ...
  if (architecture === "transformer" || architecture === "transformer_causal") {
    ...
    const nodes = [node("input", "input", text ? "Tokens [T]" : "Sequence [T,F]", 20, 160), node("embed", text ? "embedding" : "sequence_projection", text ? "Embedding 64" : "Projection 64", 220, 160, text ? { vocab_size: 4096, d_model: 64, padding_idx: 0 } : { d_model: 64 }), node("pos", "positional_encoding", "Position", 430, 160, { max_length: 256, learned: false }), node("attention", causal ? "causal_transformer" : "transformer_encoder", causal ? "Causal transformer" : "Transformer encoder", 610, 160, { heads: 4, num_layers: 2, dim_feedforward: 256, dropout: 0.1 }), ...(causal ? [] : [node("pool", "sequence_pool", text ? "First contextual token" : "Sequence pooling", 850, 160, { mode: text ? "cls" : forecast ? "last" : "mean" })]), node("head", "linear", `Output ${classes}`, causal ? 850 : 1050, 160, { out_features: classes }), ...(forecast ? [node("reshape", "reshape", "Horizon 8×1", 1240, 160, { shape: "8,1" })] : []), node("output", "output", causal ? "Logits per token" : "Result", forecast ? 1440 : (causal ? 1050 : 1240), 160)]; return sequential(nodes);
  }
  if (architecture === "vit") {
