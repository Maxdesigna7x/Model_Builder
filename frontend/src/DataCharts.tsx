...
export default function DataCharts({analytics,task}:{analytics?:DatasetAnalytics;task:TaskId}){
  ...
  return <div className="data-chart-view"><div className="data-chart-controls"><label><span>{t("charts:labels.chart")}</span><select value={kind} onChange={event=>setKind(event.target.value as ChartKind)}>{choices.map(choice=><option key={choice.id} value={choice.id}>{t(choice.label)}</option>)}</select></label>{hasFeatures&&<label><span>{t("charts:labels.variable")}</span><select value={feature} onChange={event=>setFeature(Number(event.target.value))}>{analytics.featureHistograms!.map((item,index)=><option key={item.name} value={index}>{item.name}</option>)}</select></label>}<small>{t(chartHelp(kind))}</small></div><div ref={element} className="data-echart" role="img" aria-label={t("charts:chartOf", { label: currentChoice ? t(currentChoice.label) : t("charts:labels.data") })}/></div>;
}
