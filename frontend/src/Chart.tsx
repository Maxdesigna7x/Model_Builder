...
export function TrainingChart({ history, metric, accent, compact = false }: { history: MetricPoint[]; metric: string; accent: keyof typeof accentColors; compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    const option: EChartsOption = {
      ...
      legend: { data: isLoss ? ["Train", "Validation"] : [metric], textStyle: { color: "#a8b3c2" }, top: 0 },
      grid: { left: 44, right: 16, top: 38, bottom: 30 },
      xAxis: { type: "category", name: "epoch", data: history.map(point => point.epoch), axisLabel: { color: "#768397" }, axisLine: { lineStyle: { color: "#303842" } } },
      yAxis: { type: "value", scale: true, axisLabel: { color: "#768397" }, splitLine: { lineStyle: { color: "#262c34", type: "dashed" } } },
      series: isLoss ? [
        { name: "Train", type: "line", smooth: true, showSymbol: history.length < 30, data: history.map(point => point.trainLoss), lineStyle: { color: accentColors[accent], width: 2 }, itemStyle: { color: accentColors[accent] } },
        { name: "Validation", type: "line", smooth: true, showSymbol: true, data: history.map(point => point.valLoss), lineStyle: { color: "#fbbf24", width: 2, type: "dashed" }, itemStyle: { color: "#fbbf24" } }
